...
window.BASE_URL = API_BASE.replace(/\/api$/, "");
...
async function apiFetch(endpoint, options = {}) { 
  ...
  const config = { 
    ...
    // ✅ FIX: Include credentials for cookies if needed 
    credentials: "include", 
  }; 
...
async function fetchUnreadCount() { 
  try { 
    ...
    // ✅ FIX: guard null before setting textContent 
...
async function fetchFarmerStats() { 
  try { 
    ...
    // ✅ FIX: each setter individually guards against null elements 
    const setEl = (id, val) => { 
      const el = document.getElementById(id); 
      if (el) el.textContent = val; 
    }; 

    setEl("statTotalProducts", data.totalProducts ?? 0); 
    setEl("statTotalOrders",   data.totalOrders   ?? 0); 
    setEl("statRevenue",       data.revenue ? `₹${data.revenue.toLocaleString("en-IN")}` : "₹0"); 
    setEl("statRating",        data.averageRating  ?? "—"); 
    setEl("stProd",            data.totalProducts  ?? 0); 
...
window.fetchFarmerStats  = fetchFarmerStats; 
