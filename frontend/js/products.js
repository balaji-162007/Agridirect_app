...
window.renderProductCard = function(p) {
  ...
  <div class="product-card ${isOutOfStock ? 'out-of-stock' : ''}" data-id="${p.id}" onclick="location.href='products.html?id=${p.id}'">
    <div class="product-card-img-container">
      ${productImgTag(p, "product-card-img-new")}
    ...
    <div class="product-card-body">
      ...
      <div class="product-card-farmer" onclick="event.stopPropagation(); location.href='products.html?section=farmers&search=${encodeURIComponent(p.farmer_name || '')}'" style="cursor:pointer" title="View Farmer Profile">
        ${farmerAvatarTag(p, "farmer-avatar-sm", "22px")}
...
function _initProductsPage() {
  ...
  async function _load(page = 1) {
    ...
    if (listingSec) listingSec.scrollIntoView({ behavior: 'smooth' });
  ...
  function _renderPagination() {
    ...
    el.innerHTML = Array.from({length: _state.totalPages}, (_, i) => i+1)
      .map(n => `<button class="btn ${n === _state.page ? 'btn-primary' : 'btn-ghost'} btn-sm"
          onclick="window._pgLoad(${n})">${n}</button>`).join('');
  }

  /* Expose _load globally so inline onclick on pagination buttons can reach it */
  window._pgLoad = function(page) { _load(page); };
    ...
    grid.innerHTML = items.map((f, i) => {
      ...
      <div class="farmer-card">
        <div style="position:relative;width:fit-content;margin:0 auto">
          ${farmerAvatarTag(f, "farmer-avatar-lg", "72px")}
          ${verifiedBadge}
        </div>
  ...
  async function _showMarketSection() {
    ...
    try {
      ...
      if (district) {
        const dData = await API.getMarketDistricts();
      ...
      const data = await API.getMarketPrices(Object.fromEntries(params));
          ...
          <div class="product-detail-grid">
            ...
            <div class="detail-info-col">
              ...
              <div class="detail-farmer-box mt-8">
                ${farmerAvatarTag(p, "farmer-avatar-lg", "52px")}
                <div style="flex:1">
        ...
        html = `<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px"><h3>Customer Reviews</h3><div style="display:flex;align-items:center;gap:10px"><div class="stars">${renderStars(avg)}</div><span class="fw-600">${avg.toFixed(1)} / 5</span><span class="text-muted text-sm">(${data.total} reviews)</span></div></div>` +
          ...
          <div class="review-item">
            <div class="review-head">
              ${farmerAvatarTag({name: r.customer_name, photo: r.customer_photo}, "review-avatar", "32px")}
              <div>
                <div class="fw-600 text-sm">${r.customer_name}</div>
                <div class="stars" style="font-size:.78rem">${renderStars(r.overall_service)}</div>
              </div>
              <div class="review-date">${fmtDate(r.created_at)}</div>
            </div>
            <div style="display:flex;gap:14px;font-size:.75rem;color:var(--gray-500);margin-bottom:7px">
              <span>Quality: <b>${r.product_quality}/5</b></span>
              <span>Delivery: <b>${r.delivery_time}/5</b></span>
              <span>Service: <b>${r.overall_service}/5</b></span>
            </div>
  ...
  if (_up.get('page'))   _state.page   = parseInt(_up.get('page')) || 1;
